	...
	while (!code_reader_.eof())
	{
		...
		if ('/' == ch) //处理注释
		{
			...
			else if ('*' == next) // /**/注释
			{
				while (!code_reader_.eof())
				{
					while (GetNextChar() != '*' && !code_reader_.eof())
						; //到*为止
		...
		else if (IsDigital(ch)) //可能是小数float或者整数int
		{
			str_token += ch;
			while (!code_reader_.eof())
			{
		...
		else if (ch == '\'') //开头是字符型标志\‘
		{
			...
			while (!code_reader_.eof())
			{
				...
				while (!code_reader_.eof())
				{
					if (IsDigital(ch) || IsLetter(ch) || IsBlank(ch)) //数字，字母，' ',\n,\t,255
						str_token += ch;
					else if (ch == '\'') //\'(字符结束标志)
						str_token += ch;
					else
